...
export const fetchData = (date) => {
  ...
  return request(`/data?date=${moment(date).format().slice(0, 10)}`)
}

export const fetchCountForDateString2 = async (d) => {
  const m = moment(d);
  const isoDayBefore = m.subtract(1, 'days').format().slice(0, 10);
  const isoDayAfter = m.add(2, 'days').format().slice(0, 10);
